const en = {
  "Required": "Required",
  "Expand": "Expand",
  "Collapse": "Collapse",
  "GET": "GET",
  "POST": "POST",
  "PUT": "PUT",
  "DELETE": "DELETE",
  "PATCH": "PATCH",
  "OPTIONS": "OPTIONS",
  "HEAD": "HEAD",
  "Headers": "Headers",
  "Base URL": "Base URL",
  "Parameters": "Parameters",
  "Query Parameters": "Query Parameters",
  "Path Parameters": "Path Parameters",
  "Cookie Parameters": "Cookie Parameters",
  "Header Parameters": "Header Parameters",
  "Request Body": "Request Body",
  "Response": "Response",
  "Responses": "Responses",
  "Security": "Security",
  "Components": "Components",
  "Schemas": "Schemas",
  "Security Schemes": "Security Schemes",
  "Examples": "Examples",
  "Try it out": "Try it out",
  "Send": "Send",
  "Cancel": "Cancel",
  "Loading...": "Loading...",
  "Status": "Status",
  "Error": "Error",
  "No content defined": "No content defined",
  "Failed to resolve reference": "Failed to resolve reference",
  "Optional": "Optional",
  "Default": "Default",
  "Format": "Format",
  "Pattern": "Pattern",
  "Min": "Min",
  "Max": "Max",
  "Example": "Example",
  "Schema": "Schema",
  "Type": "Type",
  "Unknown": "Unknown",
  "Authentication": "Authentication",
  "Select authentication method": "Select authentication method",
  "No authentication": "No authentication",
  "Request error": "Request error",
  "Error sending request": "Error sending request",
  "Version": "Version",
  "API Description": "API Description",
  "Terms of Service": "Terms of Service",
  "Contact": "Contact",
  "License": "License",
  "Name": "Name",
  "URL": "URL",
  "Email": "Email",
  "Identifier": "Identifier",
  "External Documentation": "External Documentation",
  "deprecated": "deprecated",
  "Loading error": "Loading error",
  "Enter OpenAPI spec URL": "Enter OpenAPI spec URL",
  "You can use the input box above to enter any valid OpenAPI spec URL, the system will load it automatically.": "You can use the input box above to enter any valid OpenAPI spec URL, the system will load it automatically.",
  "Select example API": "Select example API",
  "Request failed": "Request failed",
  "Failed to load OpenAPI spec": "Failed to load OpenAPI spec",
  "Unknown error": "Unknown error",
  "GitHub repository": "GitHub repository",
  "OAuth2 Flows": "OAuth2 Flows",
  "API Key": "API Key",
  "HTTP Authentication": "HTTP Authentication",
  "OpenID Connect": "OpenID Connect",
  "Name:": "Name:",
  "In:": "In:",
  "Scheme:": "Scheme:",
  "Bearer Format:": "Bearer Format:",
  "OpenID Connect URL:": "OpenID Connect URL:",
  "Authorization URL:": "Authorization URL:",
  "Token URL:": "Token URL:",
  "Refresh URL:": "Refresh URL:",
  "Scopes": "Scopes",
  "Scope": "Scope",
  "Description": "Description",
  "Flow": "Flow",
  // Security scheme types
  "apiKey": "API Key",
  "http": "HTTP",
  "oauth2": "OAuth2",
  "openIdConnect": "OpenID Connect",
  "mutualTLS": "Mutual TLS",

  // Server Variables translations
  "Default value:": "Default value:",
  "Available values:": "Available values:",

  // Security Section translations
  "Global Requirements": "Global Requirements",
  "API calls must satisfy ONE of the following security requirement sets:": "API calls must satisfy ONE of the following security requirement sets:",
  "Scopes:": "Scopes:",
  "(No specific scopes required)": "(No specific scopes required)",
  "Failed to resolve scheme:": "Failed to resolve scheme:",
  "Security Requirements": "Security Requirements",
  "Note: Multiple security requirements represent OR logic. Multiple schemes within a requirement represent AND logic.": "Note: Multiple security requirements represent OR logic. Multiple schemes within a requirement represent AND logic.",

  // Webhook Display translations
  "Webhook:": "Webhook:",

  // Parameter Item translations
  "required": "required",
  "explode": "explode",
  "Hide details": "Hide details",
  "Show details": "Show details",

  // ExamplesDisplay translations
  "[ExamplesDisplay] Failed to resolve example ref: %s for key %s": "[ExamplesDisplay] Failed to resolve example ref: %s for key %s",

  // MediaTypeSelector translations
  "Switching media type to: %s": "Switching media type to: %s",

  // ServerDisplay translations
  "Variables": "Variables",
  "Default:": "Default:",
  "Enum:": "Enum:",

  // SchemaConstraints translations
  "Constraints": "Constraints",
  "Minimum": "Minimum",
  "Maximum": "Maximum",
  "Min Length": "Min Length",
  "Max Length": "Max Length",
  "Min Items": "Min Items",
  "Max Items": "Max Items",
  "Unique Items": "Unique Items",
  "Min Properties": "Min Properties",
  "Max Properties": "Max Properties",
  "(exclusive)": "(exclusive)",

  // Server component translations
  "Interpolated URL:": "Interpolated URL:",
  "Server Variables:": "Server Variables:",

  // ResponsesSection translations
  "Missing component definitions required to resolve references": "Missing component definitions required to resolve references",
  "No responses defined": "No responses defined",
  "Could not resolve selected response": "Could not resolve selected response",

  // CopyButton translations
  "Copy to clipboard": "Copy to clipboard",
  "Copy failed:": "Copy failed:",

  // SecuritySchemeDisplay translations
  "In:": "In:",
  "Name:": "Name:",
  "Scheme:": "Scheme:",
  "Flows:": "Flows:",
  "Authorization URL:": "Authorization URL:",
  "Token URL:": "Token URL:",
  "Refresh URL:": "Refresh URL:",
  "Scopes:": "Scopes:",
  "OpenID Connect URL:": "OpenID Connect URL:",
  "Details for mutualTLS not specifically rendered.": "Details for mutualTLS not specifically rendered.",
  "Unknown security scheme type:": "Unknown security scheme type:",

  // Component Type translations
  "schemas": "schemas",
  "responses": "responses",
  "parameters": "parameters",
  "examples": "examples",
  "requestBodies": "requestBodies",
  "headers": "headers",
  "securitySchemes": "securitySchemes",
  "links": "links",
  "callbacks": "callbacks",
  "pathItems": "pathItems",

  // LinksSection translations
  "Links": "Links",
  "Failed to resolve link:": "Failed to resolve link:",

  // LinkItem translations
  "operationId": "operationId",
  "operationRef": "operationRef",
  "Hide Details": "Hide Details",
  "Show Details": "Show Details",
  "Server": "Server",

  // ComponentItemsList translations
  "No items available": "No items available",

  // ParametersSection translations
  "Failed to resolve parameter:": "Failed to resolve parameter:",

  // RequestBodySection translations
  "Cannot resolve request body": "Cannot resolve request body",
  "Request body has no content defined": "Request body has no content defined",

  // OpenApiLayout translations
  "Basic Information": "Basic Information",
  "Servers": "Servers",
  "All Operations": "All Operations",
  "Operations \"%s\"": "Operations \"%s\"",

  // SchemaCompositionDisplay translations
  "All Of": "All Of",
  "Any Of": "Any Of",
  "One Of": "One Of",
  "Not": "Not",
  "Reference:": "Reference:",

  // SchemaExampleView translations
  "Example Data": "Example Data",
  "Schema not defined": "Schema not defined",
  "Switched view mode to: %s": "Switched view mode to: %s",

  // ExampleDisplay translations
  "No example data provided": "No example data provided",
  "XML formatting failed:": "XML formatting failed:",

  // ComponentDetail translations
  "Please select an item from the list.": "Please select an item from the list.",
  "Error: Selected item not found.": "Error: Selected item not found.",
  "Cannot resolve security scheme reference.": "Cannot resolve security scheme reference.",
  "Error: Unknown component type": "Error: Unknown component type",

  // ResponseGroup translations
  "Response Headers": "Response Headers",

  // ResponseContentSection translations
  "Encoding": "Encoding",
  "Content Type": "Content Type",
  "Style": "Style",
  "Explode": "Explode",
  "Allow Reserved": "Allow Reserved",

  // SchemaDisplay translations
  "Unknown or underspecified schema type.": "Unknown or underspecified schema type.",
  "Array 'items' definition is missing.": "Array 'items' definition is missing.",
  "Composition Schema": "Composition Schema",

  // ResponseItem translations
  "[unknown reference]": "[unknown reference]",
  "Cannot display response %s: Reference %s failed to resolve.": "Cannot display response %s: Reference %s failed to resolve.",

  // Additional generic translations
  "Webhook": "Webhook",
  "Additional information can be found here:": "Additional information can be found here:",
  "Basic Information": "Basic Information",
  "Specification Parse Error": "Specification Parse Error",
  "Parsing specification...": "Parsing specification...",
  "No operations found with tag \"%s\"": "No operations found with tag \"%s\"",
  "External Documentation": "External Documentation",
  "Reference Object": "Reference Object",
  "API Documentation": "API Documentation",
  "Search API...": "Search API...",
  "Endpoints": "Endpoints",
  "Expand All": "Expand All",
  "Collapse All": "Collapse All",
  "Data Models": "Data Models",
  "Variables": "Variables",
  "Default value:": "Default value:",
  "Available values:": "Available values:",
  "Server Variables:": "Server Variables:",
  "Description": "Description",
  "Interpolated URL:": "Interpolated URL:",
};

export default en;
