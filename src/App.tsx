...
export default function App() {
  ...
  const [layoutType, setLayoutType] = useState<"allInOne" | "pathDetail">(
    "pathDetail",
  );
  ...
  const loadSpec = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `${t("Request failed")}: ${response.status} ${response.statusText}`,
        );
      }
      ...
      setError(
        `${t("Failed to load OpenAPI spec")}: ${err instanceof Error ? err.message : t("Unknown error")}`,
      );
        ...
        <div className="w-full flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <div className="w-full max-w-4xl flex items-center gap-3">
            ...
            <div className="relative w-40">
              <select
                onChange={(e) => {
                  const selectedExample = API_EXAMPLES.find(
                    (ex) => ex.url === e.target.value,
                  );
                  if (selectedExample) {
