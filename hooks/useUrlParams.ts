...
function useUrlParams<T extends Record<string, any>>(
  ...
  // 创建默认的参数名映射（如果未提供）
  const resolvedParamNames: Record<keyof T, string> = { ...initialState } as any;
  ...
  // 更新 URL 参数
  const updateUrlParams = useCallback((values: Partial<T>) => {
    if (!syncToUrl) return;
  ...
  // 更新状态的函数
  const setStateAndUrl = useCallback((newPartialState: Partial<T>) => {
    setState(prevState => {
