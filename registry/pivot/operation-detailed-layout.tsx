...
export const OperationDetailedLayout = React.forwardRef<HTMLDivElement, OperationDetailedLayoutProps>(
  ({ spec: inputSpec, selectedPath, selectedMethod, onSelectOperation, className }, ref) => {
    ...
    // 初始化服务器选择
    useEffect(() => {
      if (parsedSpec?.servers && parsedSpec.servers.length > 0 && !selectedServer) {
        ...
        <div ref={ref} className="flex items-center justify-center h-full p-8">
          <div className="text-center">
            ...
            <p className="text-sm text-muted-foreground">
              {t('Select a path and method from the navigation to view details')}
            </p>
          </div>
        </div>
      );
    ...
    const operation = pathItem[selectedMethod.toLowerCase() as keyof typeof pathItem];

    if (!operation || typeof operation !== 'object' || !('summary' in operation || 'description' in operation || 'parameters' in operation)) {
      return (
        <div ref={ref} className="flex items-center justify-center h-full p-8">
          <div className="text-center">
            <h3 className="text-lg font-medium text-destructive mb-2">
