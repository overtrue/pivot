  ...
  (
            ...
            case "http":
              if (scheme.scheme === "basic" && authState.http?.["basic"]) {
                requestHeaders["Authorization"] =
                  `Basic ${authState.http["basic"]}`;
              } else if (
                scheme.scheme === "bearer" &&
                authState.http?.["bearer"]
              ) {
                requestHeaders["Authorization"] =
                  `Bearer ${authState.http["bearer"]}`;
              }
              break;
            case "oauth2":
              if (authState.oauth2?.[name]?.token) {
                requestHeaders["Authorization"] =
                  `Bearer ${authState.oauth2[name].token}`;
              }
              break;
            case "openIdConnect":
              if (authState.openIdConnect?.token) {
                requestHeaders["Authorization"] =
                  `Bearer ${authState.openIdConnect.token}`;
              }
              break;
          }
        });
    ...
    const sendRequest = async () => {
      try {
        ...
        setError(
          err instanceof Error ? err.message : t("Error sending request"),
        );
      } finally {
        setIsLoading(false);
      }
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-3">
            {securitySchemes.length > 1 && (
              <div className="mb-2">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setActiveSecurityScheme(e.target.value || null)
                  }
                >
            ...
            {securitySchemes
              .filter(
                (scheme) =>
                  !activeSecurityScheme || scheme.name === activeSecurityScheme,
              )
              .map((scheme, index) => {
                const { name, scheme: schemeObj } = scheme;
                ...
                switch (schemeObj.type) {
                  case "apiKey":
                    return (
                      <div
                        key={index}
                        className="p-3 border dark:border-neutral-700 rounded bg-blue-50 dark:bg-blue-900/30"
                      >
                        <div className="text-xs font-semibold text-blue-800 dark:text-blue-300 mb-2">
                          {t("API Key")} ({schemeObj.in})
                        </div>
                        <label className="text-xs text-neutral-600 dark:text-neutral-400 block mb-1">
                          {schemeObj.name}{" "}
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            handleAuthChange(scheme, e.target.value)
                          }
                        />
                      </div>
                    );
                  ...
                  case "http":
                    if (schemeObj.scheme === "basic") {
                      return (
                        <div
                          key={index}
                          className="p-3 border dark:border-neutral-700 rounded bg-green-50 dark:bg-green-900/30"
                        >
                      ...
                      return (
                        <div
                          key={index}
                          className="p-3 border dark:border-neutral-700 rounded bg-purple-50 dark:bg-purple-900/30"
                        >
                          <div className="text-xs font-semibold text-purple-800 dark:text-purple-300 mb-2">
                            Bearer Token{" "}
                            {schemeObj.bearerFormat &&
                              `(${schemeObj.bearerFormat})`}
                          </div>
                          <label className="text-xs text-neutral-600 dark:text-neutral-400 block mb-1">
                            Token{" "}
                  ...
                  case "oauth2":
                    return (
                      <div
                        key={index}
                        className="p-3 border dark:border-neutral-700 rounded bg-orange-50 dark:bg-orange-900/30"
                      >
                  ...
                  case "openIdConnect":
                    return (
                      <div
                        key={index}
                        className="p-3 border dark:border-neutral-700 rounded bg-indigo-50 dark:bg-indigo-900/30"
                      >
    ...
    return (
      <div
        ref={ref}
        className={cn(
          "border dark:border-neutral-700 rounded-lg overflow-hidden bg-white dark:bg-neutral-800 transition-all",
          className,
        )}
      >
        <div
                    ...
                    >
                      <div className="col-span-7">
                        <div className="flex items-center gap-1">
                          <span
                            className={`text-sm ${param.required ? "font-semibold" : ""} dark:text-neutral-300`}
                          >
                            {param.name}{" "}
                            {param.required && (
                          ...
                          <span className="text-xs bg-neutral-100 dark:bg-neutral-700 px-1 py-0.5 rounded dark:text-neutral-300">
                            {param.in}
                          </span>
                        </div>
                      </div>
                  ...
                  {(() => {
                    ...
                    if (requestBody && "description" in requestBody) {
                      return (
                        ...
                        "Please enter request body data"
                      );
                    }
              ...
              <div className="mt-4 border dark:border-neutral-700 rounded-md overflow-hidden">
                <div className="bg-neutral-50 dark:bg-neutral-800/70 p-3 border-b dark:border-neutral-700 flex justify-between items-center">
                  <div className="flex items-center">
                    <span
                      className={`px-2 py-1 rounded text-xs font-medium ${getStatusStyle(response.status)}`}
                    >
