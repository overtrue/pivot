import { SecuritySchemes } from "@/registry/pivot/security-schemes";
...
export default function SecuritySchemesDemo() {
  ...
  const enterpriseSecuritySchemes = {
    "ApiKeyAuth": {
      ...
      description: "API 密钥认证，适用于服务器到服务器的通信。密钥通过 X-API-Key 请求头传递，具有访问速率限制和 IP 白名单保护。",
      name: "X-API-Key",
      in: "header" as const
    },
    "BearerAuth": {
      type: "http" as const,
      description: "JWT Bearer Token 认证，用于用户会话管理。令牌包含用户身份和权限信息，支持自动刷新和撤销机制。",
      scheme: "bearer",
      bearerFormat: "JWT"
    },
    "OAuth2": {
      type: "oauth2" as const,
      description: "OAuth 2.0 授权框架，支持多种授权流程。适用于第三方应用集成和细粒度权限控制。",
      flows: {
        authorizationCode: {
    ...
    "OpenIdConnect": {
      ...
      description: "OpenID Connect 身份认证，基于 OAuth 2.0 构建。提供标准化的身份验证和用户信息获取。",
  ...
  const webAppSecuritySchemes = {
    "SessionAuth": {
      type: "apiKey" as const,
      description: "基于 Cookie 的会话认证，适用于传统 Web 应用。包含 CSRF 保护和会话超时机制。",
      name: "sessionId",
      in: "cookie" as const
    },
    "BasicAuth": {
      type: "http" as const,
      description: "HTTP 基本认证，使用用户名和密码。仅建议在 HTTPS 环境下使用，适用于简单的内部工具。",
      scheme: "basic"
    },
    "DigestAuth": {
      type: "http" as const,
      description: "HTTP 摘要认证，比基本认证更安全。密码经过哈希处理，防止明文传输。",
      scheme: "digest"
    }
  };
  ...
  const microserviceSecuritySchemes = {
    "ServiceToken": {
      type: "http" as const,
      description: "服务间通信令牌，用于微服务架构中的服务认证。支持服务发现和负载均衡。",
      scheme: "bearer",
      bearerFormat: "JWT"
    },
    "MutualTLS": {
    ...
    "InternalApiKey": {
      type: "apiKey" as const,
      description: "内部 API 密钥，用于内网服务间的快速认证。包含服务标识和权限范围。",
      name: "X-Service-Key",
      in: "header" as const
    }
  };
  ...
  const mobileAppSecuritySchemes = {
    "MobileOAuth": {
      type: "oauth2" as const,
      description: "移动应用 OAuth 2.0 认证，支持 PKCE 扩展以增强安全性。适用于原生移动应用。",
      flows: {
        authorizationCode: {
    ...
    "DeviceAuth": {
      type: "oauth2" as const,
      description: "设备授权流程，适用于输入受限的设备（如智能电视、IoT 设备）。",
      flows: {
        clientCredentials: {
    ...
    "BiometricAuth": {
      type: "apiKey" as const,
      description: "生物识别认证令牌，结合设备本地生物识别（指纹、面部识别）生成的安全令牌。",
  ...
  const thirdPartySecuritySchemes = {
    "WebhookSignature": {
      type: "apiKey" as const,
      description: "Webhook 签名验证，确保 Webhook 请求的真实性和完整性。使用 HMAC-SHA256 算法。",
      name: "X-Webhook-Signature",
      in: "header" as const
    },
    "PartnerApiKey": {
      type: "apiKey" as const,
      description: "合作伙伴 API 密钥，用于第三方服务集成。包含合作伙伴标识和权限级别。",
      name: "X-Partner-Key",
      in: "header" as const
    },
    "SamlAuth": {
      type: "http" as const,
      description: "SAML 2.0 单点登录认证，适用于企业级身份提供商集成。支持属性映射和角色继承。",
      scheme: "bearer",
      bearerFormat: "SAML"
    }
  };
