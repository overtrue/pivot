...
const ComponentDetail: React.FC<ComponentDetailProps> = ({
  ...
  if (!item)
    return (
      <div className="text-red-500 p-4">
        {t("Error: Selected item not found.")}
      </div>
    );
  ...
  switch (activeType) {
    ...
    case "parameters": {
      ...
      return (
        <ParameterItem {...paramItem} schema={schema} components={components} />
      );
    }
    ...
    case "examples":
      ...
      return (
        <ExamplesDisplay
          examples={{ [selectedItemName]: example }}
          components={components}
        />
      );
    ...
    case "requestBodies":
      return (
        <RequestBodySection
          requestBody={item as RequestBodyObject | ReferenceObject}
          components={components}
        />
      );
    ...
    case "securitySchemes":
      const scheme = resolveRef<SecuritySchemeObject>(
        item as SecuritySchemeObject | ReferenceObject,
        components,
        "securitySchemes",
      );
      if (!scheme)
        return (
          <div className="text-red-500 p-4">
            {t("Cannot resolve security scheme reference.")}
          </div>
        );
    ...
    case "callbacks":
      return (
        <CallbackDisplay
          name={selectedItemName}
          callback={item as CallbackObject | ReferenceObject}
          components={components}
        />
      );
    ...
    default:
      return (
        <div className="text-red-500 p-4">
          {t("Error: Unknown component type")} '{activeType}'.
        </div>
      );
  }
};
