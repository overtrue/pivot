...
import { ComponentsObject, LinkObject, ReferenceObject } from "@/types/openapi"; // Adjust path
...
const LinksSection: React.FC<LinksSectionProps> = ({ links, components }) => {
  ...
  return (
    <div className="mb-4">
      ...
      <h4 className="text-sm font-semibold uppercase text-neutral-500 mb-2">
        {t("Links")}
      </h4>
      <div className="space-y-3">
        {linkEntries.map(([name, linkOrRef]) => {
          ...
          if (!link) {
            const refString =
              linkOrRef && typeof linkOrRef === "object" && "$ref" in linkOrRef
            ...
            return (
              <div
                key={name}
                className="text-xs text-red-500 p-1 border border-dashed rounded"
              >
