...
import OperationBox from "./OperationBox"; // Reuse OperationBox to display callback operations
...
const CallbackDisplay: React.FC<CallbackDisplayProps> = ({
  ...
  if (!callback) {
    ...
    return (
      <div
        className={`text-xs text-red-500 p-1 border border-dashed rounded ${className}`}
      >
    ...
    <div className={`p-4 border rounded bg-neutral-50 ${className}`}>
      ...
      {Object.entries(callback).map(([expression, pathItemOrRef]) => (
        <div key={expression} className="mb-6">
          <h4 className="text-md font-semibold mb-2 font-mono bg-neutral-200 px-2 py-1 rounded inline-block">
