...
const SchemaDisplay: React.FC<
  SchemaDisplayProps & { _currentDepth?: number }
> = ({ schema: schemaOrRef, components, _currentDepth = 0, className }) => {
  const { t } = useI18n();
  ...
  if (!resolvedSchema) {
    ...
    return (
      <div
        className={`text-xs text-red-500 dark:text-red-400 p-1 border border-dashed rounded dark:border-red-800 ${className}`}
      >
  ...
  if (_currentDepth > MAX_DEPTH) {
    return (
      <div
        className={`text-xs text-orange-500 dark:text-orange-400 p-1 border border-dashed rounded dark:border-orange-800 ${className}`}
      >
  ...
  switch (type) {
    ...
    case "null":
      return (
        <div
          className={`p-2 rounded bg-neutral-50/50 dark:bg-neutral-800/50 ${className}`}
        >
        ...
        <div className={`${className}`}>
          ...
          {items ? (
            ...
            <div className="mt-1 text-xs text-orange-500 dark:text-orange-400 ml-1">
              {t("Array 'items' definition is missing.")}
            </div>
          )}
        </div>
      );
    ...
    default:
      if (allOf || anyOf || oneOf || not) {
        return (
          <div
            className={`p-2 border rounded border-dashed border-neutral-300 dark:border-neutral-600 dark:bg-neutral-800/30 ${className}`}
          >
            <span className="text-xs text-neutral-500 dark:text-neutral-400 italic">
              {t("Composition Schema")}
            </span>
            {deprecated && <DeprecatedBadge />}
            {description && (
              <DescriptionDisplay
      ...
      return (
        <div
          className={`text-xs text-neutral-500 dark:text-neutral-400 p-1 border border-dashed rounded dark:border-neutral-600 ${className}`}
        >
