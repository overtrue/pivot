...
const SchemaConstraints: React.FC<SchemaConstraintsProps> = ({
  schema,
  className,
}) => {
  const { t } = useI18n();
  ...
  if (schema.minimum !== undefined) {
    constraints.push({
      ...
      value: (
        <>
          {schema.minimum}
          {schema.exclusiveMinimum && ` ${t("(exclusive)")}`}
        </>
      ),
    });
  }
  if (schema.maximum !== undefined) {
    constraints.push({
      ...
      value: (
        <>
          {schema.maximum}
          {schema.exclusiveMaximum && ` ${t("(exclusive)")}`}
        </>
      ),
    });
  }
  ...
  if (schema.minProperties !== undefined) {
    constraints.push({
      label: t("Min Properties"),
      value: schema.minProperties,
    });
  }
  if (schema.maxProperties !== undefined) {
    constraints.push({
      label: t("Max Properties"),
